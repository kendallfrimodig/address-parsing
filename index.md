Pre-Geocoding Address Parsing and Standardization - ED
================
Kendall Frimodig
08 June, 2021
...
patients address is transcribed during data entry from handfilled
...
address elements and basic missing elements such as directional or
...
to geocoding such as apartment number will be separated from the street
address. The program has three distinctive sections.

The preparatory phase reads in de-identified address data and assigns
variables to generic names. The rest of the code ideally does not have
to be edited, however certain sections in the manual parsing may be
manipulated depending on the address qualities.
...
The second analyzes the remaining address name with most elements parsed
...
accomplished using nested loops, that analyze each address sequentially,
splitting the address into sub-strings and scanning for identifiers.
...
pcakages only necessary once in session.**
...
and then duplicates assigning a ‘UID’ which allows for eventual linkage
...
the atypical formats of pre/post elements in the lists’ below.
...
objects (for simplicity, not having to reference the data-frame
...
‘postmstr’ parser does a good job of separating directional pre-fixes,
...
pre-directional or street suffix had not been separated from the street
...
matches for elements to be separated are set for logical statements
below. Additionally elements which would flag an address as non-standard
...
```
    ...
    ##  [1] "xxxx Burton Se St "       "xxxx Burton Se St "      
    ##  [3] "xxxx Little Big Horn Rd " "xxxx Squaw Valley Rd "     
    ##  [5] "xxxx Buckskin Rd "        "xxxx County C23 Rd "       
    ##  [7] "xxxx Hwy 434  "           "xxxx Elliott Barker  "     
